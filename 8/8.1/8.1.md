...
```
### Código Requerimiento: A-004
Interfaces:
1. Código: AI-004
![mostrarPostmor](https://github.com/user-attachments/assets/7d8641f3-bd81-4589-9f20-0901eb5ad7b7)

```sql
Sentencias:
1. Mostrar detalles postmortem:
SELECT 
    Severidad,
    COUNT(*) AS Total_Detalles,
    AVG(CAST(Recuperacion AS INTEGER)) AS Promedio_Recuperacion
FROM 
    Postmortem_detalles
GROUP BY 
    Severidad
ORDER BY 
    Promedio_Recuperacion ASC;

```
2. Código: AI-005
![registrar-post](https://github.com/user-attachments/assets/d2e3c16f-579f-40fe-a410-7dcbe3bf7e93)


```sql
Sentencias:
INSERT INTO Postmortem_detalles (
    resumen_post, 
    impacto, 
    deteccion, 
    recuperacion, 
...
### Código de requerimiento: A-005
1. Código: AI-006
![ReporteIncidente](https://github.com/user-attachments/assets/06f8a6df-da57-4ee0-b88e-0e01cd7ff883)
```sql
Sentencias:
SELECT 
    ...
    CASE 
        WHEN (COUNT(*) * 100.0 / SUM(COUNT(*)) OVER ()) > 20 THEN 'ALTO'
        ELSE 'BAJO'
    END AS GRADO_DE_INCIDENCIA
FROM 
    Incidente
GROUP BY 
    categoria
...
## Módulo de Gestión de Registro y Monitoreo de Empleados

### Código Requerimiento: E-001

Código Interfaz: EI-001

Imagen interfaz: ![image](https://github.com/user-attachments/assets/36eedc59-2dc4-45bb-b5e7-ce67878297fc)
...
1.- Cargar la pagina:
```sql
SELECT e.ID_empleado, e.Nombre, e.Apellido, e.fecha_ingreso, e.Estado,
...
JOIN Puesto p ON e.id_puesto = p.id_puesto
JOIN Departamento d ON p.id_departamento = d.id_departamento
WHERE (e.ID_empleado = COALESCE(NULL, e.ID_empleado))
  AND (e.Estado = COALESCE(NULL, e.Estado)) 
  AND (e.Nombre = COALESCE(NULL, e.Nombre))
  AND (e.Apellido = COALESCE(NULL, e.Apellido))
  AND (p.nombre = COALESCE(NULL, p.nombre))
  AND (d.descripcion = COALESCE(NULL, d.descripcion));
```
Código Interfaz: EI-002

Imagen interfaz: ![image](https://github.com/user-attachments/assets/972c1bcb-af0c-4739-93a0-92477763a76a)
...
VALUES ('David', 'Lopez', '1987-05-05', '2010-10-30', 'Activo', '567890123', '555-5678', 5)
RETURNING ID_empleado;
```
Código Interfaz: EI-003

Imagen interfaz: ![image](https://github.com/user-attachments/assets/ae4a4aa2-225b-419f-a364-4b7b62dfec64)
![image](https://github.com/user-attachments/assets/44e46dae-f780-4874-aa07-807867ea8a61)
![image](https://github.com/user-attachments/assets/c72d536e-8840-485f-b8d9-21684a8049ba)
...
SELECT 
       ...
       d.descripcion AS departamento, f.nombre AS funcion,
...
```
2.- Editar o actualizar la informacion:
```sql
UPDATE Empleado
SET 
    Nombre = COALESCE(NULL, Nombre),
    Apellido = COALESCE(NULL, Apellido),
    Documento_identidad = COALESCE(NULL, Documento_identidad),
    Telefono = COALESCE(NULL, Telefono),
    Estado = COALESCE(NULL, Estado),
    id_puesto = COALESCE(2, id_puesto)
WHERE ID_empleado = 1;
```
...
```
### Código Requerimiento: E-002

Código Interfaz: EI-004

Imagen interfaz: ![image](https://github.com/user-attachments/assets/4dcc7587-5aa3-4e18-b7fb-19e025e3ee45)

Sentencias SQL:
Eventos:

1.- Cargar la pagina: Lista de tipos de Permiso
```sql
SELECT ID_tipo_permiso, nombre 
FROM Tipo_permiso;
```
2.- Solicitud de Permiso:
```sql
INSERT INTO Permiso (fecha_inicio, fecha_final, estado, comentario, ID_empleado, ID_tipo_permiso)
VALUES ('2025-01-15', '2025-01-20', 'Pendiente', 'Licencia médica por enfermedad', 1, 2);
```
2.- Adjuntar Evidencia:
```sql
INSERT INTO EvidenciaPermiso (nombre_archivo, ruta_archivo, ID_permiso)
VALUES ('certificado_medico.pdf', '/uploads/evidencias/certificado_medico.pdf', 1);
```
Código Interfaz: EI-005

Imagen interfaz: ![image](https://github.com/user-attachments/assets/b9c5be92-5b37-44ca-b9d9-4cf1e56f62c7)

Sentencias SQL:
Eventos:

1.- Cargar la pagina: Lista de Solicitud de todos los permisos por defecto en Pendiente
```sql
SELECT p.ID_permiso, e.Nombre, e.Apellido, p.fecha_inicio, p.fecha_final, tp.nombre AS tipo_permiso,
       p.comentario, d.descripcion AS departamento, e.id_puesto
FROM Permiso p
...
```
2.- Buscar:Aplicando filtros de tipo de permiso y estados
```sql
SELECT p.ID_permiso, e.Nombre, e.Apellido, p.fecha_inicio, p.fecha_final, p.estado, 
       tp.nombre AS tipo_permiso, p.comentario, d.descripcion AS departamento, 
       pu.nombre AS puesto, e.Documento_identidad, e.Telefono
FROM Permiso p
JOIN Empleado e ON p.ID_empleado = e.ID_empleado
JOIN Tipo_permiso tp ON p.ID_tipo_permiso = tp.ID_tipo_permiso
JOIN Puesto pu ON e.id_puesto = pu.id_puesto
JOIN Departamento d ON pu.id_departamento = d.id_departamento
WHERE (p.estado = COALESCE(<estado>, p.estado))       -- Filtro opcional para el estado
  AND (tp.nombre = COALESCE(<tipo_permiso>, tp.nombre))  -- Filtro opcional para tipo de permiso
ORDER BY p.fecha_inicio DESC;
```
3.- Boton para aprobar :
```sql
UPDATE Permiso
SET estado = 'Aprobado', comentario_admin = NULL
WHERE ID_permiso = 2;

```
3.- Boton para rechazar :
```sql
UPDATE Permiso
SET estado = 'Rechazado', comentario_admin = '<razón del rechazo>'
WHERE ID_permiso = <ID_permiso>;

```
4.- Triggers para el cambio de aprobación o rechazo
```sql
...
```
Código Interfaz: EI-006

Imagen interfaz: ![image](https://github.com/user-attachments/assets/75781c2e-38a3-4f8e-85d3-aab18a9cf36f)


Sentencias SQL:

1.- Presionar en Ver mas Detalles:
```sql
SELECT p.ID_permiso, p.fecha_inicio, p.fecha_final, p.estado, 
       tp.nombre AS tipo_permiso, p.comentario AS comentario_empleado, 
...
```
Código Interfaz: EI-007

Imagen interfaz: ![image](https://github.com/user-attachments/assets/66dc0f8d-ff0c-429d-8726-bedab1a22a10)

Sentencias SQL:

1.- Vista de Mis permisos solicitados:De parte del empleado
```sql
SELECT p.ID_permiso, tp.nombre AS tipo_permiso, p.fecha_inicio, p.fecha_final, p.estado, 
       p.comentario AS comentario_empleado, p.comentario_admin AS comentario_administrador, 
       p.fecha_cambio_estado
FROM Permiso p
JOIN Tipo_permiso tp ON p.ID_tipo_permiso = tp.ID_tipo_permiso
WHERE p.ID_empleado = 2
ORDER BY 
    CASE WHEN p.estado = 'Pendiente' THEN 0 ELSE 1 END,   -- Las solicitudes pendientes primero
    p.fecha_cambio_estado DESC NULLS LAST;
```
### Código Requerimiento: E-003

Código Interfaz: EI-008

Imagen interfaz: ![image](https://github.com/user-attachments/assets/f4fe81f5-ce04-4a3c-8000-a6f9f297d24a)

Sentencias SQL:

Eventos:

1.- Registro de Hora Entrada:
```sql
INSERT INTO Asistencia (ID_empleado, fecha, hora_entrada)
VALUES (<ID_empleado>, CURRENT_DATE, CURRENT_TIME)
ON CONFLICT (ID_empleado, fecha) DO UPDAT
```
2.- Registro Hora salida :
```sql
UPDATE Asistencia
SET hora_salida = CURRENT_TIME
WHERE ID_empleado = 1 AND fecha = CURRENT_DATE;
```
```sql
...
---
CREATE OR REPLACE FUNCTION registrar_ausencias()
RETURNS VOID AS $$
BEGIN
    INSERT INTO Asistencia (ID_empleado, fecha, estado)
    SELECT e.ID_empleado, CURRENT_DATE, 'Ausente'
    FROM Empleado e
    LEFT JOIN Asistencia a ON e.ID_empleado = a.ID_empleado AND a.fecha = CURRENT_DATE
    WHERE a.ID_empleado IS NULL;
END;
$$ LANGUAGE plpgsql;
SELECT cron.schedule('registrar_ausencias_diario', '59 23 * * *', 'SELECT registrar_ausencias();');
```

### Código Requerimiento: E-004

Código Interfaz: EI-009

Imagen interfaz:

Sentencias SQL: ![image](https://github.com/user-attachments/assets/d481a6c2-c8ca-4df4-a204-54732a27c898)

Eventos:

1.- Poner Objetivos: De parte del empleado
```sql
INSERT INTO Objetivo (descripcion, estado, fecha_inicio, fecha_fin, ID_empleado)
VALUES ('Descripción del objetivo', 'Pendiente', '2024-01-01', '2024-06-30', <ID_empleado>);
```
Código Interfaz: EI-010

Imagen interfaz: ![image](https://github.com/user-attachments/assets/0c1dc226-920d-4d87-b86f-f5fdb1f94ff3)

Eventos:

1.- Cargar la pagina : Se muestran todos los objetivos pendientes para aprobar u rechazar y ponerles prioridad
```sql
SELECT o.ID_objetivo, o.descripcion, o.fecha_inicio, o.fecha_fin,
       e.Nombre AS empleado_nombre, e.Apellido AS empleado_apellido, p.nombre AS puesto
FROM Objetivo o
JOIN Empleado e ON o.ID_empleado = e.ID_empleado
JOIN Puesto p ON e.id_puesto = p.id_puesto
WHERE o.estado = 'Pendiente';
```
2.- Aprobar : Se muestran todos los objetivos pendientes para aprobar 
```sql
UPDATE Objetivo
SET estado = 'Aprobado',
    ID_Prioridad_objetivo = (SELECT ID_Prioridad_objetivo FROM Prioridad_objetivo WHERE nombre = 'Alta')
...
WHERE ID_objetivo = <ID_objetivo>;
```
Código Interfaz: EI-011

Imagen interfaz:  ![image](https://github.com/user-attachments/assets/bc0088c5-aa67-4763-8c8b-e4928c195b07)

1.- Resultados de Objetivos : 
```sql
---Registrar Resultados de Objetivos
INSERT INTO Evaluacion_objetivo (estado, ID_objetivo, ID_evaluacion)
VALUES ('Cumplido', <ID_objetivo>, <ID_evaluacion>);
```

Código Interfaz: EI-012

Imagen interfaz: ![image](https://github.com/user-attachments/assets/854af972-ba0a-44ca-9b4a-e78260926ab0)


Eventos:

1.- Creación de evaluacion : 
```sql
INSERT INTO Evaluacion (estado, fecha_inicio, fecha_fin, puntaje, retroalimentacion, ID_empleado)
VALUES ('Pendiente', '2024-06-30', '2024-06-30', 0, 'Evaluación pendiente', <ID_empleado>);
```
```sql
UPDATE Evaluacion
SET puntaje = (
               SELECT (SUM(CASE WHEN eo.estado = 'Cumplido' THEN po.peso ELSE 0 END)::FLOAT 
                       / NULLIF(SUM(po.peso), 0)) * 100
               FROM Evaluacion_objetivo eo
               JOIN Objetivo o ON eo.ID_objetivo = o.ID_objetivo
               JOIN Prioridad_objetivo po ON o.ID_Prioridad_objetivo = po.ID_Prioridad_objetivo
               WHERE eo.ID_evaluacion = Evaluacion.ID_evaluacion
           ),
    estado = 'Finalizada'
WHERE ID_evaluacion = 1;
```

Código Interfaz: EI-013

Imagen interfaz: ![image](https://github.com/user-attachments/assets/c650709d-391f-4341-95e1-bd7aa49de1db)


1.- Cargar Evaluaciones : Se muestran las evaluaciones del empleado
```sql
SELECT e.ID_evaluacion,
       ...
       e.retroalimentacion,
       STRING_AGG(o.descripcion || ' (' || eo.estado || ')', ', ') AS objetivos_evaluados
FROM Evaluacion e
JOIN Evaluacion_objetivo eo ON e.ID_evaluacion = eo.ID_evaluacion
JOIN Objetivo o ON eo.ID_objetivo = o.ID_objetivo
WHERE e.ID_empleado = 3  
GROUP BY e.ID_evaluacion, e.fecha_inicio, e.fecha_fin, e.puntaje, e.estado, e.retroalimentacion
