...
public class PrototipoService implements PrototipoServiceInterface {
    ...
    public int marcarPrototipoComoTerminado(Integer codPrototipo) {
        String sql = """
            UPDATE Prototipo
            SET 
                cod_est_prot = (
                    ...
                    WHERE estado_prot = 'Terminado'
                )
            WHERE 
                cod_prototipo = ?
        """;

        return jdbcTemplate.update(sql, codPrototipo);
    }
    
    public List<Map<String, Object>> getPrototiposByEmpleado(Integer idEmpleado) {
        String sql = """
            SELECT 
                e.Nombre || ' ' || e.Apellido AS autor, 
                p.cod_prototipo,
